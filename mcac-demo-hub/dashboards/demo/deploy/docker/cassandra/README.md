...
**Apply without the UI:** from **`dashboards/demo`**, **`./deploy/docker/cassandra/apply-scenario-hub-schema.sh`** (runs **[`ensure-scenario-hub.cql`](ensure-scenario-hub.cql)** via **`cqlsh`** on the first node). Schema is also ensured in **[`../realtime-orders-search-hub/demo-ui/scenario.py`](../realtime-orders-search-hub/demo-ui/scenario.py)**.
...
```bash
./deploy/docker/cassandra/apply-compaction-lab.sh
# optional sample rows:
./deploy/docker/cassandra/apply-compaction-lab.sh --with-samples
```
...
**Schema:** [`ensure-twcs-gc-lab.cql`](ensure-twcs-gc-lab.cql) — apply with **`./deploy/docker/cassandra/apply-twcs-gc-lab.sh`**.
...
ROWS=20000 ./deploy/docker/cassandra/run-twcs-gc-sweep.sh
```
