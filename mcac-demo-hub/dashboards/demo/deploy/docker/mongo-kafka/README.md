...
It runs at the end of **[`prepare-demo-collections.sh`](prepare-demo-collections.sh)** (Compose **`mongo-kafka-prepare`** mounts the script as **`/demo-indexes.js`**). To re-apply on a running cluster: from **`dashboards/demo`**, **`./deploy/docker/mongo-kafka/apply-demo-indexes.sh`**.
...
3. **Register connectors** (from **`dashboards/demo`**):
   ...
   ./deploy/docker/mongo-kafka/register-mongo-connectors.sh
   ```

   Optional URL: `./deploy/docker/mongo-kafka/register-mongo-connectors.sh http://localhost:8083`
...
./deploy/docker/mongo-kafka/seed-demo-items.sh           # default: 30 documents
./deploy/docker/mongo-kafka/seed-demo-items.sh 100       # custom count
```
...
```bash
MONGOS_URI="mongodb://mongo-mongos1:27017" ./deploy/docker/mongo-kafka/seed-demo-items.sh 30
```
...
3. **No topics or empty sink** — run **`mongo-kafka-prepare`** successfully once; confirm **`sh.status()`** shows **`demo`** enabled and **`demo_items`** sharded. Re-run **`./deploy/docker/mongo-kafka/register-mongo-connectors.sh`** after fixing the cluster.
