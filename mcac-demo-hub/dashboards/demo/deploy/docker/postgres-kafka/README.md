...
**Apply:** on **first** primary boot, **[`04-scenario-hub-schema-indexes.sql`](04-scenario-hub-schema-indexes.sql)** runs under **`docker-entrypoint-initdb.d`**. On an **existing** volume, from **`dashboards/demo`**: **`./deploy/docker/postgres-kafka/apply-scenario-hub-schema-indexes.sh`**. Implementation mirror: **[`../realtime-orders-search-hub/demo-ui/scenario.py`](../realtime-orders-search-hub/demo-ui/scenario.py)**.

Full per-index list: **[`../../README.md` → Hub scenario indexes](../../README.md#hub-scenario-indexes-multi-db-reference)**.

## Architecture

- **PostgreSQL (Bitnami 16, custom image `mcac-demo/postgresql-repmgr:16.6.0`):** one writable **primary** and two **read replicas** (physical streaming replication). The image is built from **[`Dockerfile.repmgr`](Dockerfile.repmgr)** on top of **`docker.io/bitnamilegacy/postgresql:16.6.0-debian-12-r2`** (same pin as Kubernetes manifests). It adds the **[repmgr](https://repmgr.org/)** CLI and **`repmgr` PostgreSQL extension** while keeping Bitnami streaming replication for this demo. **Also bundled (compiled with the same `pg_config`):** **[pg_cron](https://github.com/citusdata/pg_cron)** (jobs — metadata DB **`postgres`** via **`cron.database_name`**), **[pg_repack](https://github.com/reorg/pg_repack)** (online repack CLI + extension), **[pg_partman](https://github.com/pgpartman/pg_partman)** (partition sets / maintenance). **[pg_profile](https://github.com/zubkov-andrei/pg_profile)** (release **4.11** tarball — historic workload samples/reports; needs **`dblink`** + **`pg_stat_statements`**). Logical decoding (`wal_level=logical`) is enabled on the primary for Debezium. For production, pin an image **digest** instead of a floating tag.
- **`pg_stat_statements`:** Compose and Kubernetes set **`POSTGRESQL_SHARED_PRELOAD_LIBRARIES`** to **`repmgr,pgaudit,pg_stat_statements,pg_cron`** (repmgr first; keep **`pgaudit`** — Bitnami default). **`pg_cron`** must stay in this list on **primary and replicas** so replicated extension catalogs match; scheduling runs on the primary (standby does not execute cron jobs). **`cron.database_name=postgres`** is set via **`POSTGRESQL_EXTRA_FLAGS`** — **`CREATE EXTENSION pg_cron`** runs only in **`postgres`** (see **[`06-partman-repack-cron-extensions.sh`](06-partman-repack-cron-extensions.sh)** / **`postgres-demo-bootstrap`**). Tune **`pg_stat_statements.*`** via **`POSTGRESQL_EXTRA_FLAGS`**. On **first** primary init, **[`03-pg-stat-statements.sh`](03-pg-stat-statements.sh)** runs **`CREATE EXTENSION IF NOT EXISTS pg_stat_statements`** in database **`postgres`** only. If you see **`pg_stat_statements must be loaded via shared_preload_libraries`**, restart the container after pulling compose changes and run `SHOW shared_preload_libraries;` — it must include **`pg_stat_statements`**. **Existing volume:** env vars apply when the data dir is (re)initialized; otherwise set `shared_preload_libraries` to include **`repmgr`**, **`pgaudit`**, **`pg_stat_statements`**, and **`pg_cron`** in server config and **restart** (not reload), then recreate extensions from **`06-*`** / bootstrap SQL.
...
|--------|-------------------|
| Dockerfile | **[`Dockerfile.repmgr`](Dockerfile.repmgr)** — repmgr + **pg_cron** / **pg_repack** / **pg_partman** built from release tarballs; **pg_profile** extension files from release tarball. |
...
| Extension in the server | **`POSTGRESQL_SHARED_PRELOAD_LIBRARIES`**: **`repmgr`**, **`pgaudit`**, **`pg_stat_statements`**, **`pg_cron`** (**pg_repack** / **pg_partman** do not require preload). Optional: add **`pg_partman_bgw`** if you enable pg_partman’s background worker (not set in this demo). |
...
- **Primary only — first init:** **[`05-repmgr-extension.sh`](05-repmgr-extension.sh)** is mounted into **`docker-entrypoint-initdb.d`** on **`postgresql-primary`** only. It creates database **`repmgr`** and runs **`CREATE EXTENSION IF NOT EXISTS repmgr;`**. **[`06-partman-repack-cron-extensions.sh`](06-partman-repack-cron-extensions.sh)** creates **`pg_cron`** in **`postgres`** and **`pg_repack` / `pg_partman`** in **`demo`**. **[`08-pg-profile-extension.sh`](08-pg-profile-extension.sh)** creates **`dblink`** and **`pg_profile`** in schema **`profile`** on database **`postgres`**.
...
- **`postgresql-extensions-ensure`:** one-shot Compose service (**`restart: "no"`**) runs **`CREATE EXTENSION IF NOT EXISTS`** on the primary after **`postgresql-primary`** is healthy — applies **every** **`docker compose up`** even when PGDATA already existed (**Bitnami `docker-entrypoint-initdb.d` runs only on first empty volume**). Replicas **`depends_on`** this job **`service_completed_successfully`** so catalog DDL lands before streaming catches up.

### pg_profile (historic workload reports)

Installed in database **`postgres`**, schema **`profile`** (recommended layout from upstream docs). Depends on **`pg_stat_statements`** (preloaded) and **`dblink`**. After deploy, verify:

```bash
psql -h localhost -p 15432 -U postgres -d postgres -c "\\dx profile.*"
psql -h localhost -p 15432 -U postgres -d postgres -c "SELECT profile.take_sample();"
```

Schedule periodic samples with **pg_cron** or an external cron calling **`profile.take_sample()`**. Reports: **`SELECT profile.get_report(sample1, sample2);`** — see [pg_profile documentation](https://github.com/zubkov-andrei/pg_profile/blob/4.11/doc/pg_profile.md). Manual apply on existing clusters: **[`ensure-pg-profile.sql`](ensure-pg-profile.sql)**.
...
- Bootstrap **`postgres-demo-bootstrap`** / ConfigMap SQL mirrors Compose init: **`repmgr`**, **`pg_cron`** (**`postgres`** DB), **`pg_repack`** / **`pg_partman`** (**`demo`**), plus Debezium + scenario schema (see **`45-postgres-bootstrap-job.yaml`**).
- Logical subscriber pod **`postgres-sub`** uses **`POSTGRES_SUB_IMAGE`** (stock Bitnami) — **no** pg_cron / pg_repack / pg_partman unless you switch that image to **`mcac-demo/postgresql-repmgr:16.6.0`** and set matching **`shared_preload_libraries`** yourself.
...
| Primary (existing volume) | From `dashboards/demo`: `chmod +x deploy/docker/postgres-kafka/apply-physical-replication-slots.sh && ./deploy/docker/postgres-kafka/apply-physical-replication-slots.sh` |
...
**Existing primary volume** from before `demo` existed: as `postgres`, run **[`ensure-demo-app-user.sql`](ensure-demo-app-user.sql)** (see file header), then **`./deploy/docker/kafka-connect-register/register-all.sh`** so the JDBC sink uses `demo` again.
...
2. **Publication or `replicator` privileges missing** (common on volumes created before init was updated): apply grants + publication, then register again (from `dashboards/demo`):
   ...
   ./deploy/docker/postgres-kafka/apply-ensure-debezium.sh
   ./deploy/docker/postgres-kafka/register-connectors.sh
   ```
...
- **Existing volume:** from `dashboards/demo` run **`./deploy/docker/postgres-kafka/apply-ensure-debezium.sh`**, then restart / re-register **`pg-source-demo`** (`./deploy/docker/kafka-connect-register/register-all.sh` or delete + `register-connectors.sh`).
...
2. **Stale sink table schema** (common after iterating connector versions): from `dashboards/demo` run **[`clean-kafka-connect-sinks.sh`](../../clean-kafka-connect-sinks.sh)** — it **`DROP`**s `public.demo_items_from_kafka` so **`auto.create`** can rebuild it, then re-run **`./deploy/docker/kafka-connect-register/register-all.sh`** (or **`deploy/docker/postgres-kafka/register-connectors.sh`**).
