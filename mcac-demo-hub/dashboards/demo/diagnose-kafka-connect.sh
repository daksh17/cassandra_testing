...
for name in pg-source-demo jdbc-sink-demo mongo-source-demo mongo-sink-demo; do
  ...
  if [[ "$code" != "200" ]]; then
    echo "HTTP $code — connector may not be registered. Run: ./deploy/docker/kafka-connect-register/register-all.sh"
    continue
  fi
...
echo "If connectors list is [] or tasks FAILED, re-register: ./deploy/docker/kafka-connect-register/register-all.sh"
