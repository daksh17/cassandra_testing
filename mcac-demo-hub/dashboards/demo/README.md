...
**Shared documentation (same app on Docker Compose and Kubernetes):** **[`docs/README.md`](docs/README.md)** · [Hub & data flow](docs/hub-and-data-flow.md) · [Compose vs K8s](docs/compose-vs-kubernetes.md)
...
**Kafka Connect:** a one-shot **`kafka-connect-register`** service runs **[`deploy/docker/kafka-connect-register/register-all.sh`](deploy/docker/kafka-connect-register/register-all.sh)** so all four demo connectors are registered (**Postgres** Debezium + JDBC sink, **Mongo** Debezium + Mongo sink). **Hub UI** “Single order” / “Workload” write to **`demo_items`** (Postgres) and **`demo.demo_items`** (Mongo), which those connectors capture. Re-register from the host with **`./deploy/docker/kafka-connect-register/register-all.sh`** if Connect was restarted.
...
**Sinks stay empty after Workload / Single order:** the hub only writes the **source** tables (`public.demo_items`, `demo.demo_items`). Rows appear in **`demo_items_from_kafka`** / **`demo.demo_items_from_kafka`** only if **all four connectors are registered** and **RUNNING** (Kafka Connect → topics → sinks). Run **[`./diagnose-kafka-connect.sh`](diagnose-kafka-connect.sh)** from this directory; if connectors are missing or `FAILED`, run **`./deploy/docker/kafka-connect-register/register-all.sh`** and check **`docker compose logs kafka-connect`**.
...
| Folder | Focus |
|--------|--------|
| **[`deploy/docker/cassandra/`](deploy/docker/cassandra/README.md)** | Cassandra + MCAC + nodetool-exporter |
| **[`deploy/docker/kafka/`](deploy/docker/kafka/README.md)** | ZooKeeper, Kafka broker, Kafka Connect image |
| **[`deploy/docker/observability/`](deploy/docker/observability/README.md)** | Prometheus + Grafana provisioning |
| **[`deploy/docker/redis/`](deploy/docker/redis/README.md)** | Redis **7** (password, AOF, volume) |
| **[`deploy/docker/opensearch/`](deploy/docker/opensearch/README.md)** | OpenSearch single-node + Dashboards (dev, HTTP) |
| **[`deploy/docker/postgres-kafka/`](deploy/docker/postgres-kafka/README.md)** | PostgreSQL HA + Debezium + JDBC sink |
| **[`deploy/docker/mongo-kafka/`](deploy/docker/mongo-kafka/README.md)** | Sharded Mongo + Debezium + Mongo sink |
| **[`deploy/docker/mongo-sharded/`](deploy/docker/mongo-sharded/README.md)** | Init scripts for config/shard/mongos (`tic` / `tac` / `toe`) |
| **[`deploy/docker/realtime-orders-search-hub/`](deploy/docker/realtime-orders-search-hub/README.md)** | **Reference scenario**: Postgres + Mongo CDC, Kafka, Cassandra, Redis, OpenSearch, observability (workflows + Mermaid diagrams) |
| **[`deploy/k8s/`](deploy/k8s/README.md)** | **Kubernetes** — same demo-hub stack as **`docker-compose.yml`**, generated by **`deploy/k8s/scripts/gen_demo_hub_k8s.py`** (**Secrets**, Cassandra **PVCs**, **Ingress / PDB / HPA / NetworkPolicy / CronJob**, optional NodePort & ServiceMonitor example). Entry points: **`deploy/k8s/scripts/demo-hub.sh`**, **`deploy/k8s/scripts/stop-start-all-k8s.sh`**, **`deploy/k8s/scripts/port-forward-demo-hub.sh`**. |
| **[`deploy/docker/`](deploy/docker/README.md)** | **Docker Compose assets** — SQL/scripts, hub UI source, Kafka Connect image context, per-area READMEs (`docker-compose.yml` stays here so **`../prometheus`**, **`../../config`**, and **`build: ../../`** stay valid). |

### Kubernetes (same stack on a cluster)

The **`dashboards/demo/deploy/k8s/`** tree mirrors Compose: **`deploy/k8s/scripts/demo-hub.sh start`** applies **`deploy/k8s/generated/all.yaml`** (namespace **`demo-hub`**) and runs **`deploy/k8s/scripts/apply-data-bootstrap.sh`** (Postgres / Cassandra / Mongo Jobs). **What is different from “plain” demo YAML:** credentials live in a **Secret**; Cassandra uses **PersistentVolumeClaims**; **`96-kubernetes-ops.yaml`** adds **Ingress**, **PodDisruptionBudget**, **HPA**, **NetworkPolicy**, and a **CronJob**; the hub UI uses **headless Cassandra DNS** for CQL contact points.

**Step-by-step** (build local images → start pods → port-forward → optional NodePort / Connect registration): **[`deploy/k8s/README.md` — Quick start (step by step)](deploy/k8s/README.md#quick-start-step-by-step)**.

**From your laptop**, ClusterIP services are reached via **`deploy/k8s/scripts/port-forward-demo-hub.sh`** (or Ingress if you install a controller). See **[`deploy/k8s/README.md`](deploy/k8s/README.md)** for **`LOCAL_*`** port overrides (**`LOCAL_CQL_PORT`** for Cassandra on localhost), Cassandra workarounds, and **`stop-start-all-k8s.sh`** defaults (**`WITH_PORT_FORWARD=0`**).
...
|--------|-----------------|---------------------------|
| PostgreSQL | [`deploy/docker/realtime-orders-search-hub/demo-ui/scenario.py`](deploy/docker/realtime-orders-search-hub/demo-ui/scenario.py) (`ensure_postgres_scenario_schema`, `_ensure_postgres_scenario_indexes`) + [`deploy/docker/postgres-kafka/04-scenario-hub-schema-indexes.sql`](deploy/docker/postgres-kafka/04-scenario-hub-schema-indexes.sql) (initdb + parity) | [`deploy/docker/postgres-kafka/apply-scenario-hub-schema-indexes.sh`](deploy/docker/postgres-kafka/apply-scenario-hub-schema-indexes.sh) |
| MongoDB | [`deploy/docker/mongo-kafka/demo-indexes.js`](deploy/docker/mongo-kafka/demo-indexes.js) (run from [`deploy/docker/mongo-kafka/prepare-demo-collections.sh`](deploy/docker/mongo-kafka/prepare-demo-collections.sh) or apply script) | [`deploy/docker/mongo-kafka/apply-demo-indexes.sh`](deploy/docker/mongo-kafka/apply-demo-indexes.sh) |
| Cassandra | [`deploy/docker/realtime-orders-search-hub/demo-ui/scenario.py`](deploy/docker/realtime-orders-search-hub/demo-ui/scenario.py) (`ensure_cassandra_scenario_schema`) + [`deploy/docker/cassandra/ensure-scenario-hub.cql`](deploy/docker/cassandra/ensure-scenario-hub.cql) | [`deploy/docker/cassandra/apply-scenario-hub-schema.sh`](deploy/docker/cassandra/apply-scenario-hub-schema.sh) |
