...
"$DEMO_ROOT/k8s/scripts/build-mcac-init-image.sh"

echo "=== 2/4 Hub demo UI ==="
docker build -t mcac-demo/hub-demo-ui:latest -f realtime-orders-search-hub/demo-ui/Dockerfile realtime-orders-search-hub/demo-ui

echo "=== 3/4 Kafka Connect (Debezium + Mongo sink) ==="
docker build -t mcac-demo/kafka-connect:2.7.3-mongo-sink -f mongo-kafka/Dockerfile.connect mongo-kafka

echo "=== 4/4 Nodetool exporter ==="
docker build -t demo-hub/nodetool-exporter:latest -f nodetool-exporter/Dockerfile nodetool-exporter
